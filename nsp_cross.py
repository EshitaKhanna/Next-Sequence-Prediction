...
print("Loading raw data HHAR - Data:", hhar_data.shape, "Labels:", hhar_label.shape)
print("Unique activity labels in HHAR - ", np.unique(hhar_label[:,0,2])) # activity label at index 2
print("Before normalization HHAR - Data shape:", hhar_data.shape, hhar_data[0][0][:])
hhar_normalizer = Preprocess4Normalization(feature_len=6)
hhar_normalized_data = np.zeros_like(hhar_data)

for i in range(len(hhar_data)):
    hhar_normalized_data[i] = hhar_normalizer(hhar_data[i])

print("After normalization HHAR- Data:", hhar_normalized_data.shape, "Labels:", hhar_label.shape, hhar_normalized_data[0][0][:])

print("\nNormalized Samples HHAR")
for i in range(117, 120):
    print(hhar_normalized_data[0][i][:])

np.save('dataset/hhar/normalized_data_20_120.npy', hhar_normalized_data)

# Shoaib
shoaib_data = np.load("dataset/shoaib/data_20_120.npy")
shoaib_data = shoaib_data[:, :, :6]  # using only first 6 features instead of 9 
shoaib_label = np.load("dataset/shoaib/label_20_120.npy")

print("Loading raw data Shoaib - Data:", shoaib_data.shape, "Labels:", shoaib_label.shape)
print("Unique activity labels in Shoaib - ", np.unique(shoaib_label[:,0,0])) # activity label at index 0
print("Before normalization Shoaib - Data shape:", shoaib_data.shape, shoaib_data[0][0][:])

shoaib_normalizer = Preprocess4Normalization(feature_len=6)
shoaib_normalized_data = np.zeros_like(shoaib_data)

for i in range(len(shoaib_data)):
    shoaib_normalized_data[i] = shoaib_normalizer(shoaib_data[i])

print("After normalization Shoaib- Data:", shoaib_normalized_data.shape, "Labels:", shoaib_label.shape, shoaib_normalized_data[0][0][:])

print("\nNormalized Samples Shoaib")
for i in range(117, 120):
    print(shoaib_normalized_data[0][i][:])

np.save('dataset/shoaib/normalized_data_20_120.npy', shoaib_normalized_data)
...
def train_lstm(input_len, predict_len, train_base_dataset, train_split_info, test_base_dataset, test_split_info):
    log_hyperparameters(input_len, predict_len)  

    config_name = f"input_{input_len}_pred_{predict_len}"
    base_dir = f"lstm_cross_nsp/{config_name}"
    ...
    # HHAR train/val/test splits
    train_loader, val_loader, hhar_test_loader, hhar_pair_splits = prepare_data_loaders(
        train_base_dataset, train_split_info, input_len, predict_len
    )

    test_dataset = Dataset4NSP(test_base_dataset, input_len, predict_len)
    shoaib_test_loader = DataLoader(test_dataset, batch_size=BATCH_SIZE)

    # save test indices 
    np.save(f"{model_name}_hhar_test_pairs.npy", hhar_pair_splits['test_pairs'])
    print(f"Saved HHAR test pairs: {len(hhar_pair_splits['test_pairs'])}")

    shoaib_test_pairs = np.arange(len(test_dataset))  # All pairs are test
    np.save(f"{model_name}_shoaib_test_pairs.npy", shoaib_test_pairs)
    print(f"Saved Shoaib test pairs: {len(shoaib_test_pairs)}")

    test_loader = shoaib_test_loader

    print(f"Train (HHAR): {len(train_loader.dataset)}, Val (HHAR): {len(val_loader.dataset)}, Test (Shoaib): {len(shoaib_test_loader.dataset)} ")
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch {epoch+1}/{num_epochs}, Train Loss: {avg_loss:.4f}, Val Loss: {avg_val_loss:.4f}, Cross-Dataset Test Loss: {avg_test_loss:.4f}")
    ...
    if all_preds:  # Check if we have predictions to save
        ...
        np.save(f"{model_name}_predictions.npy", preds_array)
        np.save(f"{model_name}_actuals.npy", actuals_array)
        print(f"Saved Cross-Dataset LSTM-NSP predictions: {preds_array.shape}, actuals: {actuals_array.shape}")
...
def compare_models(pairs):
    ...
    for input_len, pred_len in pairs:
        config_name = f"input_{input_len}_pred_{pred_len}"
        test_losses = np.load(f"lstm_corss_nsp/{config_name}/lstm_nsp_{input_len}_{pred_len}_test_losses.npy")
...
if __name__ == "__main__":
    ...
    hhar_filtered_data, hhar_kept_indices, hhar_removed_indices = process_dataset(hhar_normalized_data, pad=True) # HHAR
    shoaib_filtered_data, shoaib_kept_indices, shoaib_removed_indices = process_dataset(shoaib_normalized_data, pad=True) # Shoaib
    
    # Save the processed data
    os.makedirs("processed_data", exist_ok=True)
    np.save("processed_data/hhar_filtered_data.npy", hhar_filtered_data)
    np.save("processed_data/shoaib_filtered_data.npy", shoaib_filtered_data)
    
    # Save indices as pickle files to handle varying-length lists
    import pickle
    with open("processed_data/hhar_kept_indices.pkl", 'wb') as f:
        pickle.dump(hhar_kept_indices, f)
    with open("processed_data/hhar_removed_indices.pkl", 'wb') as f:
        pickle.dump(hhar_removed_indices, f)
    
    with open("processed_data/shoaib_kept_indices.pkl", 'wb') as f:
        pickle.dump(shoaib_kept_indices, f)
    with open("processed_data/shoaib_removed_indices.pkl", 'wb') as f:
        pickle.dump(shoaib_removed_indices, f)
    
    # Create consistent dataset splits
    hhar_base_dataset, hhar_split_info = prepare_dataset_splits(hhar_filtered_data, hhar_kept_indices) # HHAR
    shoaib_base_dataset, shoaib_split_info = prepare_dataset_splits(shoaib_filtered_data, shoaib_kept_indices) # Shoaib
    ...
    for input_len, pred_len in INPUT_OUTPUT_PAIRS:
        print(f"\n===== Training LSTM Model (Cross Dataset) with Input Length {input_len}, Prediction Length {pred_len} =====")
        model = train_lstm(input_len, pred_len, hhar_base_dataset, hhar_split_info, shoaib_base_dataset, shoaib_split_info)
    ...
    for input_len, pred_len in INPUT_OUTPUT_PAIRS:
        ...
        model = train_model_limu(input_len, pred_len, hhar_base_dataset, hhar_split_info)
