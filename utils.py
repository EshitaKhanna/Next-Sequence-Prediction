...
def merge_dataset(data, label, mode='all'):
    print(f"merge_dataset: BEFORE - data shape: {data.shape}, labels shape: {label.shape}, mode: {mode} ")
    ...
    result_data, result_label = data[index], np.array(label_new)
    print(f"merge_dataset: AFTER - data shape: {result_data.shape}, labels shape: {result_label.shape}")
    return data[index], np.array(label_new)


def reshape_data(data, merge):
    print(f"reshape_data: BEFORE - data shape: {data.shape}, merge: {merge}")
    if merge == 0:
        result = data.reshape(data.shape[0] * data.shape[1], data.shape[2])
    else:
        result = data.reshape(data.shape[0] * data.shape[1] // merge, merge, data.shape[2])
    print(f"reshape_data: AFTER - new shape: {result.shape}")
    return result


def reshape_label(label, merge):
    print(f"reshape_label: BEFORE - label shape: {label.shape}, merge: {merge}")
    if merge == 0:
        result = label.reshape(label.shape[0] * label.shape[1])
    else:
        result = label.reshape(label.shape[0] * label.shape[1] // merge, merge)
    print(f"reshape_label: AFTER - new shape: {result.shape}")
    return result


def shuffle_data_label(data, label):
    print(f"shuffle_data_label: BEFORE - data shape: {data.shape}, label shape: {label.shape}")
    index = np.arange(data.shape[0])
    np.random.shuffle(index)
    
    print(f"shuffle_data_label: AFTER - data shape: {data[index, ...].shape}, label shape: {label[index, ...].shape}")
    return data[index, ...], label[index, ...]


def prepare_pretrain_dataset(data, labels, training_rate, seed=None):
    print(f"prepare_pretrain_dataset: BEFORE - data shape: {data.shape}, labels shape: {labels.shape}, training_rate: {training_rate}, seed: {seed}")
    set_seeds(seed)
    data_train, label_train, data_vali, label_vali, data_test, label_test = partition_and_reshape(data, labels, label_index=0
                                                                                                  ...
                                                                                                  , change_shape=False)
    print(f"prepare_pretrain_dataset: AFTER - data_train shape: {data_train.shape}, label_train shape: {label_train.shape}, data_vali shape: {data_vali.shape}, label_vali shape: {label_vali.shape}")
...
def prepare_classifier_dataset(data, labels, label_index=0, training_rate=0.8, label_rate=1.0, change_shape=True
    ...
    print(f"prepare_classifier_dataset: BEFORE - data shape: {data.shape}, labels shape: {labels.shape}, label_index: {label_index}, training_rate: {training_rate}, label_rate: {label_rate}")
    print(f"change_shape: {change_shape}, merge: {merge}, merge_mode: {merge_mode}, seed: {seed}, balance: {balance}")
    set_seeds(seed)
    ...
    print(f"prepare_classifier_dataset: AFTER - data_train_label shape: {data_train_label.shape}, label_train_label shape: {label_train_label.shape}, data_vali shape: {data_vali.shape}, label_vali shape: {label_vali.shape}, data_test shape: {data_test.shape}, label_test shape: {label_test.shape}")
...
def partition_and_reshape(data, labels, label_index=0, training_rate=0.8, vali_rate=0.1, change_shape=True
    ...
    print(f"partition_and_reshape: BEFORE - data shape: {data.shape}, labels shape: {labels.shape}")
    ...
    print(f"partition_and_reshape: AFTER - train size: {label_train.shape[0]}, vali size: {label_vali.shape[0]}, test size: {label_test.shape[0]}")
...
def prepare_simple_dataset(data, labels, training_rate=0.2):
    print(f"prepare_simple_dataset: BEFORE - data shape: {data.shape}, labels shape: {labels.shape}, training_rate: {training_rate}")
    arr = np.arange(data.shape[0])
    np.random.shuffle(arr)
    data = data[arr]
    labels = labels[arr]
    ...
    print(f"prepare_simple_dataset: AFTER - data_train shape: {data_train.shape}, label_train shape: {label_train.shape}, data_test shape: {data_test.shape}, label_test shape: {label_test.shape}")
...
def prepare_simple_dataset_balance(data, labels, training_rate=0.8):
    print(f"prepare_simple_dataset_balance: BEFORE - data shape: {data.shape}, labels shape: {labels.shape}, training_rate: {training_rate}")
    labels_unique = np.unique(labels)
    label_num = []
    for i in range(labels_unique.size):
        label_num.append(np.sum(labels == labels_unique[i]))
    ...
    print(f"prepare_simple_dataset_balance: AFTER - data_train shape: {data_train.shape}, label_train shape: {label_train.shape}, data_test shape: {data_test.shape}, label_test shape: {label_test.shape}")
...
def match_labels(labels, labels_targets):
    print(f"match_labels: BEFORE - labels shape: {labels.shape}, labels_targets shape: {labels_targets.shape}")
...
def load_raw_data(args):
    ...
    print("Loading raw data: Data: ", data.shape, "Labels: ", labels.shape)
    return data, labels
...
# Add Next Sequence Prediction preprocessing
class Preprocess4NSP:
    """Pre-processing for Next Sequence Prediction"""
    def __init__(self, predict_len=5, input_len=85):
        self.predict_len = predict_len
        self.input_len = input_len
        self.total_len = input_len + predict_len
    
    def __call__(self, instance):
        """
        Split the instance into input sequence and target sequence
        
        Args:
            instance: Input sequence of shape [sequence_length, feature_dim]
            
        Returns:
            input_seq: Input sequence portion
            target_pos: Target positions
            target_seq: Ground truth for target sequence
        """
        # Make sure instance is long enough
        if len(instance) < self.total_len:
            # Pad if necessary
            padded = np.zeros((self.total_len, instance.shape[1]))
            padded[:len(instance)] = instance
            instance = padded
        
        # Take only the part we need (in case it's longer)
        instance = instance[:self.total_len]
        
        # Split into input and target
        input_seq = instance[:self.input_len].copy()
        target_seq = instance[self.input_len:].copy()
        
        # Target positions are the next predict_len positions after input
        target_pos = np.arange(self.input_len, self.total_len)
        
        return input_seq, target_pos, target_seq


# Dataset for Next Sequence Prediction
class LIBERTDataset4NSP(Dataset):
    """Dataset for Next Sequence Prediction"""
    def __init__(self, data, pipeline=[]):
        super().__init__()
        self.pipeline = pipeline
    ...
    def __getitem__(self, index):
        ...
        for proc in self.pipeline:
            instance = proc(instance)
        input_seq, target_pos, target_seq = instance
        return torch.from_numpy(input_seq).float(), torch.from_numpy(target_pos).long(), torch.from_numpy(target_seq).float()
