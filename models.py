...
# Modified BERT model for Next Sequence Prediction
class LIMUBertModel4NSP(nn.Module):
    """LIMU-BERT model adapted for Next Sequence Prediction with variable length"""
    def __init__(self, cfg, predict_len=20, output_embed=False):
        super().__init__()
        self.transformer = Transformer(cfg)  # encoder
        ...
        # Additional layers to improve long-range prediction
        self.temporal_proj = nn.Linear(cfg.hidden, cfg.hidden)
        
        # Decoder for next sequence prediction
        self.decoder = nn.Linear(cfg.hidden, cfg.feature_num)
    
    def forward(self, input_seqs, target_pos=None):
        # Get transformer embeddings for input sequence
        h = self.transformer(input_seqs)

        if getattr(self, 'output_embed', False):
            return h
        
        # For NSP, we need to predict the next sequences
        batch_size = input_seqs.size(0)
        
        if target_pos is None:
            # During inference, predict next positions after input sequence
            seq_len = input_seqs.size(1)
            target_pos = torch.arange(seq_len, seq_len + self.predict_len, 
                                     dtype=torch.long, device=input_seqs.device)
            target_pos = target_pos.unsqueeze(0).expand(batch_size, self.predict_len)
         
        # Get the last few hidden states
        last_n = min(25, h.size(1))  # Use last 25 hidden states or fewer if sequence is shorter
        last_states = h[:, -last_n:]
    
        h_target = []
        for i in range(self.predict_len):
            # Create position-dependent weights (more recent states have higher weight)
            weights = torch.softmax(torch.arange(last_n, device=h.device, dtype=torch.float).flip(0), dim=0)
            weights = weights.view(1, -1, 1)
            
            # Create a weighted combination of last hidden states, adjusted for position
            pos_factor = 1.0 - 0.1 * min(i, 9)  # Decrease confidence for further predictions
            comb_state = (last_states * weights).sum(dim=1, keepdim=True) * pos_factor
            
            # Project through temporal layer to adjust for future time step
            time_state = self.temporal_proj(comb_state)
            h_target.append(time_state)
        
        # Concatenate all positional predictions
        h_target = torch.cat(h_target, dim=1)
        
        # Apply transformations
        h_target = self.activ(self.linear(h_target))
        h_target = self.norm(h_target)
        
        # Decode to feature space
        logits_nsp = self.decoder(h_target)
