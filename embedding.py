...
from models import LIMUBertModel4NSP, Transformer, gelu, LayerNorm
from utils import load_pretrain_data_config, get_device, handle_argv, Preprocess4Normalization, IMUDataset


def fetch_setup_nsp(args, output_embed):
    """Setup for NSP model embedding generation"""
    ...
    # Create NSP model 
    model = LIMUBertModel4NSP(model_cfg, predict_len=predict_len, output_embed=output_embed)
    
    criterion = nn.MSELoss(reduction='none')
    return data, labels, data_loader, model, criterion, train_cfg


def generate_nsp_embedding(args, save=False, output_embed=True):
    """Generate embeddings from an NSP model"""
    data, labels, data_loader, model, criterion, train_cfg = fetch_setup_nsp(args, output_embed)
    ...
    def func_forward(model, batch):
        seqs, label = batch
        # Get embeddings from the transformer encoder
        embed = model(seqs)
        return embed, label

    # For NSP model, we need to handle loading differently
    # Use load_self=True if you have a custom load method in your model
    output = trainer.run(func_forward, None, data_loader, args.pretrain_model, load_self=False)
    
    if save:
        save_name = 'embed_nsp_' + args.model_file.split('.')[0] + '_' + args.dataset + '_' + args.dataset_version
        np.save(os.path.join('embed', save_name + '.npy'), output)
    return data, output, labels


def load_nsp_embedding_label(model_file, dataset, dataset_version):
    """Load embeddings and labels for an NSP model"""
    embed_name = 'embed_nsp_' + model_file + '_' + dataset + '_' + dataset_version
...
if __name__ == "__main__":
    ...
    print("Generating embeddings from NSP model...")
    data, output, labels = generate_nsp_embedding(args=args, output_embed=True, save=save)
    print(f"Embedding shape: {output.shape}")
    print("Embeddings generated and saved successfully!")

    # Uncomment if you want to visualize the embeddings
    # label_index = 1
